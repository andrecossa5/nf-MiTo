...
Our `afm_input` file remains the same (no addition/removal of any job from the EXPLORE run).
Our `user.settings` change to:
...
We can launch the final INFER run:
...
This combination of workflows is designed to maximize e
